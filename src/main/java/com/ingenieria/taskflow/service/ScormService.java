...
public class ScormService {
    ...
    private String buildScormInitJs() {
        return "/**\n"
             ...
             + " * Inicializa la sesión con el LMS y reporta el progreso del estudiante.\n"
             + " * Compatible con: Moodle, Canvas, Blackboard, ILIAS, TalentLMS\n"
             + " */\n"
             + "(function () {\n"
             + "  'use strict';\n"
             + "\n"
             ...
             + "    api = findAPI(window);\n"
             + "    if (!api) {\n"
             + "      console.info('[TaskFlow SCORM] API no encontrada. Ejecutando en modo standalone.');\n"
             + "      return;\n"
             + "    }\n"
             + "    var r = api.LMSInitialize('');\n"
             + "    var freshInit = (r === 'true' || r === true);\n"
             + "    // Si LMSInitialize falla puede ser porque ya fue inicializado\n"
             + "    // en login.html (navegación entre páginas). La sesión sigue activa.\n"
             + "    initialized = true;\n"
             + "    if (freshInit) {\n"
             + "      // Solo inicializar valores en la primera carga (login.html)\n"
             + "      api.LMSSetValue('cmi.core.lesson_status', 'incomplete');\n"
             + "      api.LMSSetValue('cmi.core.score.raw', '0');\n"
             + "      api.LMSSetValue('cmi.core.score.min', '0');\n"
             + "      api.LMSSetValue('cmi.core.score.max', '100');\n"
             + "      api.LMSCommit('');\n"
             + "      console.log('[TaskFlow SCORM] Sesión iniciada correctamente.');\n"
             + "    } else {\n"
             + "      console.log('[TaskFlow SCORM] Reconectado a sesión existente del LMS.');\n"
             + "    }\n"
             + "  }\n"
             + "\n"
             + "  /**\n"
             + "   * Llamado desde script.js cada vez que se completa un quiz.\n"
             ...
             + "\n"
             + "    // Registrar interacción individual\n"
             ...
             + "\n"
             + "    // Marcar como passed si el promedio >= 60\n"
             + "    if (promedio >= 60) {\n"
             + "      api.LMSSetValue('cmi.core.lesson_status', 'passed');\n"
             + "    }\n"
             + "\n"
             + "    api.LMSCommit('');\n"
             + "    console.log('[TaskFlow SCORM] Quiz reportado. Promedio: ' + promedio + '% | Aprobados: ' + quizzesAprobados);\n"
             + "  }\n"
             + "\n"
             + "  /** Cierra la sesión con el LMS (llamar al salir) */\n"
             ...
             + "  }\n"
             + "\n"
             + "  // Cerrar sesión SCORM al abandonar la página\n"
             + "  window.addEventListener('beforeunload', scormFinish);\n"
             + "\n"
             + "  // API pública para que script.js pueda llamar\n"
